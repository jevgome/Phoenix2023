...
public class MasterAuto extends LinearOpMode {
    ...
    boolean one,two,wait,blue,close,left,parkBool;
    @Override
    public void runOpMode() throws InterruptedException {
        ...
        if(!isStopRequested()) {
            ...
            TrajectorySequence spikeClose = drive.trajectorySequenceBuilder(startPose)
                    ...
                    .waitSeconds(0.4)
                    .back(4)
                    .UNSTABLE_addTemporalMarkerOffset(-0.6, this::closeClaw)
                    .build();
...
//                    .lineToConstantHeading(new Vector2d(startPose.getX(),startPose.getY()))
//                    .lineToConstantHeading(new Vector2d(-58,startPose.getY()))
//                    .lineToConstantHeading(new Vector2d(drive.getPoseEstimate().getX(),(blue ? 12 : -12)))
//                    .lineToConstantHeading(new Vector2d(35,drive.getPoseEstimate().getY()))
//                    .build();


            if(wait)drive.followTrajectorySequence(waitSeq);
            drive.followTrajectorySequence(blue&&one || !blue&&!(one||two) ? spikeClose : two ? spikeMid : spikeFar);
//            if(!close) drive.followTrajectorySequence(door);
            TrajectorySequence toBackdrop = drive.trajectorySequenceBuilder(drive.getPoseEstimate())
                    .lineToLinearHeading(new Pose2d(42,35 * (blue ? 1 : -1), Math.toRadians(0)))
                    .UNSTABLE_addTemporalMarkerOffset(-1, () -> arm(2300,3000))
                    ...
                    .waitSeconds(0.6)
                    .lineToConstantHeading(new Vector2d(44,(35 + (one ? 8 : two ? 0 : -8)) * (blue ? 1 : -1)))
                    .UNSTABLE_addTemporalMarkerOffset(0, () -> {
                        stopper.setPosition(0.075);
                    })
                    .waitSeconds(0.6)
                    .back(3)
            ...
            TrajectorySequence park = drive.trajectorySequenceBuilder(drive.getPoseEstimate())
                    .lineToLinearHeading(new Pose2d(38,blue&&left ? 60 : blue ? 10 : left ? -10 : -60))
                    .forward(24)
                    .build();
